...
public class LoginService {
    ...
    public MailVerifyFlow createMailVerifyFlow(String token, String email, AccountAuthorization authorization, Identify identify) {
        MailVerifyFlow flow = new MailVerifyFlow(
                RandomUtils.string(6),
                TimeUtils.getMilliUnixTime() + Expire.MINUTE * 5,
                email,
                authorization,
                identify
        );

        verifies.put(token, flow);

        return flow;
    }

    public MailVerifyFlow getVerifyFlow(String token) {
        return verifies.get(token);
    }
