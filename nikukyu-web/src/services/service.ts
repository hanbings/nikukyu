...
const list = async <R>(api: string): Promise<R> => (await requester.get<R>(`${config.api}/${api}`)).data
const get = async <R>(api: string): Promise<R> => (await requester.get<R>(`${config.api}/${api}`)).data
const create = async <T, R>(api: string, data: T): Promise<R> =>
    (await requester.post<R>(`${config.api}/${api}`, data)).data
const update = async <T, R>(api: string, data: T): Promise<R> =>
    (await requester.post<R>(`${config.api}/${api}`, data)).data
const remove = async <R>(api: string): Promise<R> =>
    (await requester.delete<R>(`${config.api}/${api}`)).data
